...
// serial sparse implementation of pagerank. data1.dat
// R(t+1) = d M R(t) + (1-d)/N
// d - damp1, N = 4
// damp2 - (1-d)/N
// prnew <- R
...
//comment out the next two lines if node numbers start with zero		
		colindex = colindex - 1;
		link = link - 1;
		rowind[i] = link;
...
//print pagrank vector
	printf("Printing first ten elements of pagerank vector\n");
	for(j = 0; j< 10; j++) {
		printf("%f\t",prnew[j]);
	}
	printf("\nTime for solutin loop %f seconds", time_spent);
	return 0;
}
