...
int main(int argc, char **argv)
{
    ...
    /* ----------------------
       SET OPENMP THREADS FROM COMMAND LINE
    ---------------------- */
    int omp_threads = 1;
    if (argc >= 2) {
    ...
    // Verify the actual number of threads
    #pragma omp parallel
    {
        #pragma omp single
    ...
    if (rank == MASTER) {
        ...
        printf("MPI Processes: %d, OpenMP Threads per process: %d, Total Threads: %d\n", 
    ...
    /* ----------------------
       ITERATION WITH OPENMP
    ---------------------- */
    int iteration_count = 0;
    do {
        // Reset local array (parallelized)
        #pragma omp parallel for
        for (i = 0; i < N; i++)
            local[i] = 0.0;

        /* CSR COMPUTE WITH OPENMP */
        #pragma omp parallel for
        for (i = start; i < end; i++) {
            int j;
    ...
    // Each rank computes sum of PR values in its partition (parallelized)
    ...
    if (rank == MASTER) {
        ...
        if (seq_file != NULL) {
            if (fscanf(seq_file, "%lf", &seq_time) == 1) {
                ...
                double efficiency = measure_efficiency(speedup, size * omp_threads);
